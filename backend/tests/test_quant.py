...
def test_baseline_rollups_match_sub_buckets():
    ...
    assert w.bond_total == w.bond_short + w.bond_long
...
def test_subcategory_split_uses_configured_shares():
    w = quant.baseline_weights(70)
    # passive should be roughly 2/3 of etf_total per default share (allow rounding slack)
    assert abs(w.etf_passive - round(w.etf_total * 2 / 3)) <= 1
    assert abs(w.bond_long - round(w.bond_total * 2 / 3)) <= 1
