...
# ── 서비스 맥락 설명 — LLM 판정·설명 프롬프트에 주입 ──────────────
SERVICE_CONTEXT = """Quill은 증권사 리서치 리포트를 근거로 채권·ETF 중심 자산배분을
해설하는 금융 정보 서비스다. 다루는 범위:
- 자산배분: 현금성, 패시브/액티브 ETF, 단기채/장기채/회사채
- 관련 개념: 금리, 채권, 듀레이션, 자산배분 이론(MPT, 코어-새틀라이트 등), 투자 용어 전반
- 리포트 기반 시황 해설, 포트폴리오 근거 설명
다루지 않는 범위: 코인/선물/옵션 등 개별 투기성 자산, 개별 종목 추천,
금융과 무관한 일반 주제, 확정적 시세 예측."""
